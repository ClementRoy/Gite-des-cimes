...
function destroy(wizard, options)
{
    ...
    wizard.unbind(eventNamespace).removeData("uid").removeData("options")
        ...
        .find(".steps-actions a").unbind(eventNamespace);
    ...
    wizard.find(".steps-content > [data-mode='async'],.content > [data-mode='iframe']").empty();
...
function refreshPagination(wizard, options, state)
{
    ...
    {
        var finish = wizard.find(".steps-actions a[href$='#finish']").parent(),
            next = wizard.find(".steps-actions a[href$='#next']").parent();

        if (!options.forceMoveForward)
        {
            var previous = wizard.find(".steps-actions a[href$='#previous']").parent();
...
function registerEvents(wizard, options)
{
    ...
    wizard.find(".steps-actions a").bind("click" + eventNamespace, paginationClickHandler);
}
...
function render(wizard, options, state)
{
    ...
    var wrapperTemplate = "<{0} class=\"{1}\">{2}</{0}>",
        ...
        contentWrapper = $(format(wrapperTemplate, options.contentContainerTag, "steps-content " + options.clearFixCssClass, wizard.html())),
...
function renderPagination(wizard, options, state)
{
    if (options.enablePagination)
    {
        var pagination = "<{0} class=\"steps-actions {1}\"><ul role=\"menu\" aria-label=\"{2}\">{3}</ul></{0}>",
            buttonTemplate = "<li><a href=\"#{0}\" class=\"btn-glow primary\" role=\"menuitem\">{1}</a></li>",
            buttons = "";
